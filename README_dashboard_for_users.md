...
├─ for_users.py               ← 사용자용 대시보드 '어휘 판독기' (인터랙티브)
...
| 그래프 | matplotlib 정적 | plotly 인터랙티브 (3D 산점도, 레이더) |
| 특징 | 근거 추적 가능 | 원고 위 교정 표시 · 슬라이더 실험 |

`for_users.py`의 화면 이름은 **어휘 판독기(Lexis Reader)** 입니다.
사전과 교정지의 형태를 빌려 왔습니다 — 잉크 남색, 교정 표시의 적갈색, 뉴스프린트 지면.
서체는 Fraunces(라틴 표제) + 고운바탕(한글 표제) + IBM Plex Sans KR(본문) +
IBM Plex Mono(수치)를 웹폰트로 불러옵니다. 인터넷이 끊긴 환경에서는
시스템 서체로 대체되며, 레이아웃은 그대로 유지됩니다.

**핵심 화면은 표시된 원고입니다.** 점수 숫자가 아니라, 사용자가 쓴 문장 위에
저빈도 어휘(잉크색 밑줄)와 사전에 없는 단어(적갈색 물결 밑줄)를 직접 표시합니다.
"어떤 단어가 점수를 만들었나"를 자기 글에서 바로 보게 하는 것이 이 도구의 요점입니다.
